...
| `src/plan.ts` | pipeline `toPlayerState() → resolveGoalTasks(meta goals) → buildPlan(horizon, weights)` + per-raid foresight warnings + 16-hex content hash + `mapNames` (map id → display name for every planned raid, added 2026-07-11 so no consumer renders raw ids); per-horizon cache; `state.changed` → ~1.5 s debounce → rebuild → `plan.updated` (hash-gated, rebuild failures logged not thrown) |
